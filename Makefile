...
$(PREFIX)/bin/elf2x68k: tools/elf2x68k.c | $(PREFIX)/bin
	$(L0)"build elf2x68k"$(L1) $(CC) -Wall -O2 -o $@ $< $(L2)

$(PREFIX)/bin/hudson-bridge: tools/hudson-bridge.c | $(PREFIX)/bin
	$(L0)"build hudson-bridge"$(L1) $(CC) -Wall -O2 -o $@ $< $(L2)

$(PREFIX)/bin/run68: $(BUILD)/run68x/run68 | $(PREFIX)/bin
	@install -s $< $@
...
check-torture:
	@if ! grep -q 'boards_dir.*testsuite/boards' $(BUILD)/gcc/gcc/site.exp 2>/dev/null; then \
		if [ ! -f $(BUILD)/gcc/gcc/site.exp ]; then \
			echo '## All variables above are generated by configure. Do Not Edit ##' > $(BUILD)/gcc/gcc/site.exp; \
		fi; \
